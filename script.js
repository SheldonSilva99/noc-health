...
function closePopup() {
  ...
  const smsBox = document.getElementById('sms-links');
  if (smsBox) smsBox.innerHTML = '';
}
...
function removerContato(id) {
  const contatos = getStore(STORE_KEYS.contatos, []).filter(c => String(c.id) !== String(id));
  setStore(STORE_KEYS.contatos, contatos);
  renderContatos();
}
...
function removerRemedio(id) {
  let remedios = getStore(STORE_KEYS.remedios, []);

  remedios = remedios.filter(r => String(r.id) !== String(id));

  setStore(STORE_KEYS.remedios, remedios);

  const tomado = getRemediosTomados();
  delete tomado.items[id];
  delete tomado.items[String(id)];
  setStore(STORE_KEYS.remediosTomados, tomado);

  clearReminder(id);
  clearReminder(String(id));

  renderRemedios();
  updateDashboard();

  showPopup('Remédio removido', 'O remédio foi removido com sucesso.', '🗑️');
}

function marcarTomado(id) {
  const remedioId = String(id);
  setRemedioTomado(remedioId, true);
  clearReminder(remedioId);
  renderRemedios();
  updateDashboard();
...
function scheduleMedicationReminder(remedio) {
  const remedioId = String(remedio.id);
  clearReminder(remedioId);

  reminderTimeouts[remedioId] = setTimeout(async () => {
    let attempt = 1;

    const tomadoAgora = !!getRemediosTomados().items[remedioId];
    if (tomadoAgora) {
      clearReminder(remedioId);
      renderRemedios();
      return;
    ...
    reminderIntervals[remedioId] = setInterval(async () => {
      const tomado = !!getRemediosTomados().items[remedioId];
...
function renderRemedios() {
  ...
  list.innerHTML = remedios.map(r => {
    const remedioId = String(r.id);
    const tomado = !!tomados[remedioId];
    const agendado = !!(
      reminderTimeouts[remedioId] ||
      reminderIntervals[remedioId] ||
      reminderTimeouts[r.id] ||
      reminderIntervals[r.id]
    );
    ...
    return `
      <div class="history-item">
        ...
        <div style="display:flex; gap:8px; margin-top:10px; flex-wrap:wrap;">
          <button type="button" class="btn-primary" onclick="marcarTomado('${remedioId}')">✅ Tomado</button>
          <button type="button" class="btn-danger" onclick="removerRemedio('${remedioId}')">🗑️ Remover</button>
        </div>
      </div>
    `;
  }).join('');
...
function updateDashboard() {
  ...
  const tomados = getRemediosTomados().items;
  const remedios = getStore(STORE_KEYS.remedios, []);
  const pendente = remedios.some(r => !tomados[String(r.id)]);
...
function abrirSmsParaContatos(mensagem) {
  ...
  if (smsBox) {
    smsBox.innerHTML = contatos.map(c => {
      const numero = String(c.telefone || '').trim();
      const href = `sms:${encodeURIComponent(numero)}?body=${encodeURIComponent(mensagem)}`;
      return `<a class="btn-primary sms-btn" href="${href}">📩 SMS para ${escapeHtml(c.nome)}</a>`;
    }).join('');
  }
}
