...
function renderCadastro() {
  ...
  if (welcome) welcome.textContent = cadastro?.nome || 'NOC Health';
  const saved = document.getElementById('cadastro-salvo');

  if (saved) saved.classList.toggle('hidden', !cadastro);
  if (!display) return;
  if (!cadastro) {
  ...
  document.getElementById('cad-nome').value = cadastro.nome || '';
  document.getElementById('cad-idade').value = cadastro.idade || '';
  document.getElementById('cad-condicoes').value = cadastro.condicoes || '';
  document.getElementById('cad-medicamentos').value = cadastro.medicamentos || '';
  document.getElementById('cad-obs').value = cadastro.obs || '';
}
...
function renderContatos() {
  ...
  list.innerHTML = contatos.map(c => `
    <div class="history-item">
      ...
      <div style="margin-top:10px"><button class="btn-secondary" onclick="removerContato('${c.id}')">Remover</button></div>
    </div>
  `).join('');
}
  ...
  try {
    ...
    if (reg) {
      await reg.showNotification(title, {
        ...
        badge: 'https://cdn-icons-png.flaticon.com/512/2966/2966486.png',
        tag: 'noc-health',
        renotify: true,
...
async function triggerMedicationAlert(remedio, attempt) {
  ...
  await showSystemNotification('💊 Lembrete de remédio', msg);
...
function renderPressao() {
  const arr = getStore(STORE_KEYS.pressao, []);
  const containers = [document.getElementById('pressao-historico'), document.getElementById('hist-pressao')].filter(Boolean);
  const html = arr.length ? arr.map(p => `
    <div class="history-item">
      <strong>${p.max}/${p.min}</strong> — ${p.max >= 14 || p.min >= 9 ? '🚨 Alta' : '✅ Normal'}<br>
      ${formatDate(p.data)}
    </div>`).join('') : '<p class="hint-text">Nenhuma medição registrada.</p>';
...
function renderGlicemia() {
  const arr = getStore(STORE_KEYS.glicemia, []);
  const containers = [document.getElementById('glicemia-historico'), document.getElementById('hist-glicemia')].filter(Boolean);
  const html = arr.length ? arr.map(g => `
    <div class="history-item">
      <strong>${g.valor} mg/dL</strong><br>
      ${escapeHtml(g.obs || 'Sem observações')}<br>
      ${formatDate(g.data)}
    </div>`).join('') : '<p class="hint-text">Nenhuma glicemia registrada.</p>';
  containers.forEach(c => c.innerHTML = html);
}
...
function renderDiario() {
  const arr = getStore(STORE_KEYS.diario, []);
  const containers = [document.getElementById('diario-historico'), document.getElementById('hist-diario')].filter(Boolean);
  const html = arr.length ? arr.map(d => `
    <div class="history-item">
      ...
      ${formatDate(d.data)}
    </div>`).join('') : '<p class="hint-text">Nenhum registro diário salvo.</p>';
  containers.forEach(c => c.innerHTML = html);
  const dataEl = document.getElementById('diario-data');
  if (dataEl) dataEl.textContent = formatDate(nowIso());
}
...
function renderAlertas() {
  ...
  if (!c) return;
  c.innerHTML = arr.length ? arr.map(a => `
    <div class="history-item">
      <strong>${escapeHtml(a.tipo)}</strong><br>
      ${escapeHtml(a.mensagem)}<br>
      ${formatDate(a.data)}
    </div>`).join('') : '<p class="hint-text">Nenhum alerta registrado.</p>';
}
...
function updateDashboard() {
  ...
  if (boxPressao) boxPressao.classList.add(!p ? 'warn' : (p.max >= 14 || p.min >= 9 ? 'alert' : 'ok'));
  if (boxGlicemia) boxGlicemia.classList.add(!g ? 'warn' : (g.valor < 70 || g.valor >= 126 ? 'alert' : g.valor >= 100 ? 'warn' : 'ok'));
  if (boxRemedio) boxRemedio.classList.add(!remedios.length ? 'warn' : (pendente ? 'warn' : 'ok'));
  if (boxHumor) boxHumor.classList.add(!diario ? 'warn' : (/😔|😣/.test(diario.humor) ? 'alert' : 'ok'));

  const allGood = p && g && !pendente && diario && p.max < 14 && p.min < 9 && g.valor >= 70 && g.valor < 126 && !/😔|😣/.test(diario.humor);
  if (tudoCerto) tudoCerto.classList.toggle('hidden', !allGood);
}
...
function abrirSmsParaContatos(mensagem) {
  ...
  // O navegador/OS decide se abre o app de SMS
...
function obterLocalizacao() {
  ...
  if (status) status.textContent = 'Solicitando localização...';
  ...
  navigator.geolocation.getCurrentPosition(pos => {
    ...
    if (status) status.textContent = messages[err.code] || 'Erro ao obter localização.';
...
function atualizarUltimoAcessoUI() {
  ...
  if (el) el.textContent = ultimo ? `Última interação no app: ${formatDate(ultimo)}` : 'Ainda sem interação registrada.';
  if (badge) {
...
function verificarInatividade() {
  ...
  if (hora < 8 || hora > 22) return; // ignora período de sono
...
function limparHistorico() {
  ...
  [STORE_KEYS.pressao, STORE_KEYS.glicemia, STORE_KEYS.diario, STORE_KEYS.alertas].forEach(k => localStorage.removeItem(k));
  renderHistoricos();
  updateDashboard();
...
function initialRender() {
  ...
  if (today) today.textContent = new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' });
  renderCadastro();
  renderContatos();
  renderRemedios();
  renderHistoricos();
